<?php
	...
	class Kml extends Log {
		private $file = false;
		private $xml = false;

		function __construct ($file = false) {
			parent::__construct("KML");
			$this->log("Creating new KML parse engine", 0);
			$this->xml = false;
			$this->file = &$file;
		}
		...
		public function parse ($file = false) {
			...
			if ($xml === false) {
				$this->log("Failed to read XML data", 3);
				return false;
			}
		...
		public function generateKml ($data = false, $file = false) {
			if (!is_array($data)) {
				$this->log("Incorrect datatype for KML", 1);
				return false;
			}
			if ($file !== false && !is_string($file)) {
				if ($file !== false) {
					$this->log("File data type is invalid", 2);
				}
				return false;
			}
